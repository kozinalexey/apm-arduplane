...
//	void runEstmation();
//	void LocHistoryPush(    int32_t rawlat, int32_t rawlng);
//	bool LocHistoryIsFull();
//	void CalcSpeedByHistory();
//	void estItemHist( int level);
//	void bubbleSortLat();
//	void bubbleSortLng(); 
	...
	//uint8_t PEsize = 9; //5 , 7, 9  parallel estmation array size. position calculation.  5 - low latency,  9 - better filtration,
    //int32_t kcnt = 7; //count points in avg speed mode. speed calculation. 2 low latency with fast accelerations, 5 better avg 

	#define constrain(amt,low,high) ((amt)<(low)?(low):((amt)>(high)?(high):(amt)))
	//#define LocHistorySize  20  //must to be more than kcnt*2 + 1

	//Location1 LocHistory[LocHistorySize]; //location history buffer
    //uint8_t LocHistoryLen = 0;
   // uint32_t last_history_time_week_ms;
	
	

	//int32_t parallelEstmationslat[LocHistorySize + 1] ; //internal variable
    //int32_t parallelEstmationslng[LocHistorySize + 1] ; //internal variable
	//int32_t avgspeedLat = 0;
    //int32_t avgspeedLng = 0;



//locatons history buffer
