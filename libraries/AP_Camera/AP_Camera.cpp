...
const AP_Param::GroupInfo AP_Camera::var_info[] PROGMEM = {
    ...
    // @Values: 0:Servo setup servo in rc function,1:Relay1 2:relay2 relays channels may be configured in relay params
    // @User: Standard
    ...
    AP_GROUPINFO("TRIGG_DIST",  4, AP_Camera, _trigg_dist, 0),
	 ...
	 // @Values: -1:Disabled, or arduino pin
	 // @User: Standard
    ...
    if (_trigger_counter) {
        ...
        switch (_trigger_type) {
            case AP_CAMERA_TRIGGER_TYPE_SERVO:
                ...
                break;
            case AP_CAMERA_TRIGGER_TYPE_RELAY:
                _apm_relay->off(_trigger_type -1);
                break;
        }
    }
}
...
int AP_Camera::get_feedback_pin(void)
{

	return (int) _feedback_pin;
}